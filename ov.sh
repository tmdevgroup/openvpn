...
function generalBackup() {
	...
	echo "   Распаковка бэкапа успешно выполнено!"
	echo ""
	read -rp "   Автоматизировать бэкап? y-n:  " -e CHOOSEAUTOMATION
...
function newClient() {
	TODAYDATE=$(date +%m-%d)
                echo ""
                echo "Хочешь что-бы имя было с датой?:"
                echo "   1) ДА"
                echo "   2) НЕТ"
                read -rp "1-2:  " -e -i 1 CHOOSENAME
        if [[ $CHOOSENAME == '1' ]]; then
                until [[ $CLIENT =~ ^[a-zA-Z0-9_-]+$ ]]; do
                        read -rp "Имя клиента: " -e CLIEN
                        CLIENT=$CLIEN-$TODAYDATE
                done
                elif [[ $CHOOSENAME == '2' ]]; then
                        until [[ $CLIENT =~ ^[a-zA-Z0-9_-]+$ ]]; do
